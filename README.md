...
跨平台、轻量、高性能的 Rust 截图工具，基于 [egui](https://github.com/emilk/egui) 构建。程序启动后常驻系统托盘，通过托盘菜单或全局快捷键进入 Snipaste 风格的一体化截图浮层：自动识别前台窗口、自由框选、即时标注，确认后默认写入剪贴板。
...
- **标注**：矩形、椭圆、直线、箭头（细线 / 实心两种）、铅笔、文字、序号（自增）、马赛克
- **系统托盘**：截图、设置、退出；录屏与 OCR 菜单为后续能力占位
- **设置**：编辑三组全局快捷键、默认保存目录、图片格式和自动剪贴板
...
- 启动后常驻系统托盘；点击托盘「截图」或按全局快捷键 → 冻结全屏进入截图浮层
- 移动鼠标：自动高亮光标下窗口为默认选区
